...
FusionEKF::FusionEKF() {
  ...
  H_laser_ << 1, 0, 0, 0, 0, 1, 0, 0; // output selection matrix
  ekf_.P_ = MatrixXd(4, 4);           // Create process covariance matrix
  ekf_.P_ << 10, 0, 0, 0, 0, 10, 0, 0, 0, 0, 100, 0, 0, 0, 0, 100; // Initialize
}
...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    // first measurement
    cout << "EKF: " << endl;
    ekf_.x_ = VectorXd(4); // Create the state vector
    ekf_.x_ << 1, 1, 1, 1;
  ...
  ekf_.Q_ << dt_4_4 * noise_ax, 0, dt_3_2 * noise_ax, 0, 0, dt_4_4 * noise_ay,
      0, dt_3_2 * noise_ay, dt_3_2 * noise_ax, 0, dt_2 * noise_ax, 0, 0,
